...
public class GameWindow extends JFrame {
    ...
    public GameWindow(Basic basic, Spaceship spaceship){
        ...
        public void run(){
            while(movE.isAlive()){
                try {
                    if(bullet.getMov()==true){
                        bullet.move();
                        bullet.getlbl().setBounds(bullet.getXpos(),bullet.getYpos(),80,60);
                        contenedor.add(bullet.getlbl());;
                        movE.join(20);
                    }
                    else{
    ...
    public void paintComponent(Graphics g){
      temp2=basic.getEnemy(5);
      int pos = GameWindow.getCont(5);
      if(temp2==null){
          g.drawImage(spaceship.getImage(),spaceship.getXpos(),spaceship.getYpos(),110,110,this);
      }
      else if(temp2.getXpos()+pos-30<bullet.getXpos() & bullet.getXpos()<temp2.getXpos()+pos+30 & temp2.getYpos()-30<bullet.getYpos() & bullet.getYpos()<temp2.getYpos()+30 & bullet.getMov()==true ){
          limitX+=110;
